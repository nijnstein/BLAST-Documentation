...
public class Sample7 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //
        // compile it, by enabling SSMD during prepare the package can be used in SSMD runmode 
        // - if compiled for SSMD it may not contain any jump instructions (loops, ifthen)
        // 

        if (script.Prepare(true, true) != BlastError.success)
        {
